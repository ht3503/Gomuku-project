...
def is_win(board):
    ...
    PLAYER = score_of_col(board,'b')
    AI = score_of_col(board,'w')
    
    sum_sumcol_values(PLAYER)
    sum_sumcol_values(AI)
    
    if 5 in PLAYER and PLAYER[5] == 1:
        return 'PLAYER won'
    elif 5 in AI and AI[5] == 1:
        return 'AI won'

    if sum(PLAYER.values()) == PLAYER[-1] and sum(AI.values()) == AI[-1] or possible_moves(board) == []:
        return 'Draw'
...
def click(x,y):
    ...
    if board[y][x] == ' ':
        ...
        if game_res in ["AI won", "PLAYER won", "Draw"]:
            print (game_res)
            win = True
            return
...
def initialize(size):
    ...
    colors['w'].color('lightblue')  
    colors['b'].color('yellow')
